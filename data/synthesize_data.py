...
class DataSynthesizer:
  def __init__(self, data_file, backgrounds_file, max_frequency=25):
  ...
  def is_underfilled(self):
    ...
    return len(underfilled_classes) > 0

  def load_classes_sample(self, max_sample_size=5):
    underfilled_classes = [classs for classs, freq in self.classes if freq < self.max_frequency]
    sample_classes = random.sample(underfilled_classes, random.randint(1, max_sample_size))
  ...
  def synthesize(self):
    image_data_list = []
    background_info, background = self.load_background_sample()
...
if __name__ == "__main__":
  ...
  logging.info(f'Finished synthesizing images. Generated {len(synthesized_data)} images.')